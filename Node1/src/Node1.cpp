...
uint64_t measureLatency(F&& func) {
    ...
    return std::chrono::duration_cast<std::chrono::miliseconds>(t1 - t0).count();
}
...
//    set_cpu_affinity(11);  // 예: 코어 0에 고정
    ...
    csv_file << "iteration,operation_type,ecu_id,sec_level,key_len_bytes,latency_ms,status\n";
...
//    auto exec_start = Clock::now();
    ...
    for (int iter = 0; iter < ITERATIONS_FOR_MAIN_LOOP; ++iter) {
        ...
        auto total_duration_ms = std::chrono::duration_cast<millis>(total_end - total_start).count();

        csv_file << "TOTAL,TotalExecutionTime,-,-,-,"  << total_duration_ms << ",success\n";
    }
